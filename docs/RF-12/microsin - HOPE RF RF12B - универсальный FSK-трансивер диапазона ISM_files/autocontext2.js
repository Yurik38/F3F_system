...
Begun.Scripts.importAllScripts({"acp/begun_template.30657.js":true,"acp/begun_utils.36408.js":true});
...
Begun.DOM_TIMEOUT=1;
Begun.REVISION='$LastChangedRevision: 36620 $';
...
var reModule=new RegExp("^"+_this.Strings.urls.base_scripts_url.toString("/","\/")+"(\\w+)\.js$");
...
var ext={
"begun_tpl_block_120x600":"36547",
"begun_tpl_block_160x600":"36547",
"begun_tpl_block_200x300":"36547",
"begun_tpl_block_240x400":"36547",
"begun_tpl_block_468x60":"36547",
"begun_tpl_block_728x90":"36396",
"begun_tpl_block_flat":"36582",
"begun_tpl_block_horizontal":"36547",
"begun_tpl_block_rich":"36547",
"begun_tpl_block_square":"36547",
...
var BLOCK_ID_BANNER_600x90=1;
var BLOCK_ID_TOP_MOBILE=2;
var FAKE_BLOCKS=[BLOCK_ID_BANNER_600x90,BLOCK_ID_TOP_MOBILE];
...
ppcalls_counter:'http://autocontext.begun.ru/ppcallcounter?data={{ppcall_data}}&log_ppcall_visibility=1',
...
if((_block)&&(_block.id==BLOCK_ID_BANNER_600x90||_block.id==BLOCK_ID_TOP_MOBILE)){
...
var block=_this.Blocks.getBlockById(this.block_id,feed.blocks);
if(block){
window.begun_extra_block.options.banners_count=block.options.banners_count;
window.begun_extra_block.options.banners_count_coef=block.options.banners_count_coef;
window.begun_extra_block.options.capacity=block.options.capacity;
window.begun_extra_block.options.view_type=block.options.view_type;
...
comma=",";
_this.requestParams.block_id=BLOCK_ID_BANNER_600x90+comma+BLOCK_ID_TOP_MOBILE;
...
var min=70;
var max=200;
var isInFrame=window!=window.top;
if(_this.Blocks.checkType(block,'vertical')){
options={
is_equal:true
}
if(isInFrame){
options.left_max_default=10;
options.top_max_default=-30;
max=125;
}else{
options.left_max_default=-27.5;
options.top_max_default=-67.5;
}
...
if(isInFrame){
options.left_max_default=-10;
options.top_max_default=-8;
max=88;
}else{
options.left_max_default=-66;
options.top_max_default=-64;
}
...
if(isInFrame){
options.left_max_default=-10;
options.top_max_default=-10;
max=120;
}else{
options.left_max_default=-50;
options.top_max_default=-50;
}
...
if(window.begun_block_ids){
if(typeof _this.currentIdIndex==="undefined"){
...
}else if(typeof window.begun_block_id!=="undefined"&&window.begun_block_id>0){
...
var FAKE_BLOCK_OFFSET=200;
window.begun_block_id=FAKE_BLOCK_OFFSET+parseInt(total_banners);
}
...
var fetchBanners=function(block_id,type){
var i=0;
var obj=[];
var banner=null;
...
var codes=fetchBanners(block_id,'code');
var graphs=fetchBanners(block_id,'graph');
if(codes){
...
if(!arguments.callee.banner_600x90_inserted&&block_id==BLOCK_ID_BANNER_600x90){
var html=_this.getGraphHTML(graphs[i],null,600,90);
_this.prepareTopGraphBanner(block.id);
_this.dispatchBlockDrawCallback(block);
arguments.callee.banner_600x90_inserted=true;
}else{
if(!arguments.callee.top_mobile_inserted&&block_id==BLOCK_ID_TOP_MOBILE&&_this.getFeed().params.is_mobile){
...
if(!arguments.callee.top_mobile_inserted&&block_id==BLOCK_ID_TOP_MOBILE&&_this.getFeed().params.is_mobile&&fetchBanners(BLOCK_ID_TOP_MOBILE,'autocontext').length){
...
this.prepareTopGraphBanner=function(block_id){
try{
var vars={id:_this.Strings.css.block_prefix+block_id};
var banner_wrapper=top.document.getElementById('begun_top_graph_banner_wrapper');
if(!banner_wrapper){
var bo=top.document.getElementsByTagName('BODY');
var banner_wrapper=top.document.createElement('div');
banner_wrapper.id='begun_top_graph_banner_wrapper';
bo[0].insertBefore(banner_wrapper,bo[0].firstChild);
}
banner_wrapper.innerHTML=(new Begun.Template(_this.Tpls.getHTML('top_graph_banner'))).evaluate(vars);
}catch(e){}
};
...
for(var i=0;window.begunAds&&window.begunAds.blocks&&i<window.begunAds.blocks.length;i++){
...
for(var i=0;i<FAKE_BLOCKS.length;i++){
...
if(obj!==null&&typeof obj!=="undefined"&&Begun.Browser.IE&&Begun.Browser.less(7)&&obj.borderColor&&(obj.borderColor.toLowerCase()=='transparent'||obj.borderColor=='')){
obj.borderColor="white";
obj.filter="chroma(color=white)";
...
vars.is_mobile=_this.getFeed(pad_id).params.is_mobile;
...
var DEFAULT_STYLE=" margin: 8px auto 5px 7px !important;";
...
css_thumbnails:Number(block.options.show_thumbnails)&&!Number(block.options.show_favicons)?_this.Strings.css.thumb+' '+(_this.Thumbs.getType()=='classic'?_this.Strings.css.thumb_classic:_this.Strings.css.thumb_default):'',
...
if(banner.block_id&&banner.block_id==block.id&&!banner.disabled){
banner_html='';
...
this.loadModules=function(block,pad){
...
var bannersContainViewType=function(view_type,section){
var section=section||'autocontext';
var hasViewType=false;
for(var i=0;i<_this.getBanners(pad.pad_id)[section].length;i++){
if(_this.checkBannerViewType(_this.getBanners(pad.pad_id)[section][i],view_type)){
hasViewType=true;
break;
}
}
...
}else if(_this.Blocks.checkType(block,'rich')||_this.Blocks.checkViewType(block,'rich')||bannersContainViewType('rich')){
...
var css={};
css['default']='\
.begun_adv *, .begun_adv div, .begun_adv *:link, .begun_adv *:visited, .begun_adv *:hover, .begun_adv *:active {\
...
';
html['bnnr_ppcall']='\
<span class="begun_adv_phone_wrapper"><a href="javascript:void(0)" onclick="'+ac.Strings.js.ppcall_show+'({{banner_index}}, this, event, {{pad_id}}, \'{{link}}\', {{is_mobile}})">{{phone}}<span class="begun_adv_card" title="&#1047;&#1074;&#1086;&#1085;&#1086;&#1082;&#32;&#1073;&#1077;&#1089;&#1087;&#1083;&#1072;&#1090;&#1085;&#1099;&#1081;">{{ppcall_text}}</span></a></span>\
';
html['bnnr_domain']='\
...
html['top_graph_banner']='<table id="begun_top_graph_banner"><tbody><tr><td class="begun_top_graph_banner_left_col"></td><td id="{{id}}"></td><td class="begun_top_graph_banner_right_col"></td></tr></tbody></table>';
...
html['search_banner_img']='<a href="{{url}}" target="_blank"><img src="{{source}}&redir=1" border="0" width="{{width}}" height="{{height}}" style="width:{{width}}px;height:{{height}}px;" /></a>';
...
res+=separatePads+"{\"pad_id\":"+pagePads[i].pad_id+",\"blocks\":[";
separateBlocks="";
...
if(ppcalls.length>2){
Begun.Utils.includeCounter(ac.Strings.urls.ppcalls_counter,{'ppcall_data':encodeURIComponent(ppcalls)});
}
if(!Begun.Browser.IE){
